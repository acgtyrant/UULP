...
extern void do_more(int pagelen , FILE *fp, FILE *fp_tty);
extern int see_more(int pagelen, FILE *fp_tty);


int main(int argc, char *argv[]) {
  struct winsize wbuf;
  if (ioctl(0, TIOCGWINSZ, &wbuf)) {
    perror("ioctl");
    exit(EXIT_FAILURE);
  }
  int pagelen = wbuf.ws_row;
  ...
  if (argc == 1)
    do_more(pagelen, stdin, fp_tty);
  else 
    while (--argc) {
      if ((fp = fopen(*++argv, "r")) != NULL) {
        do_more(pagelen, fp, fp_tty);
        fclose(fp);
      }
      else {
